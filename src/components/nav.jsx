...
export const NavSub = ({
  ...
  return (
    <nav
      className={`h-6 sticky ${top} ${zidx} justify-around flex gap-2  rounded-b-md`}
    >
      <ul
        id={nav_id}
        className="flex gap-1 text-sm font-bold text-black dark:text-white rounded-md"
      >
        {nav_items.map((nav) => (
          <li key={nav.name + " _Nav"} className=" px-1 rounded-t-md">
            <a href={`#${nav.href}`}>{nav.name}</a>
          </li>
        ))}
      </ul>
